...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// Because the internal variable and the nested function are called within the myFunction() function and as a result, all variables within a function are accessible to nested functions belonging to same function.
...
function summation(number) {
  ...
  for (let i = 0; i<=number; i++){
    sumArray.push(i);
  } /* I could easily do a summation in this for loop by looping sumVal += i with sumVal outside of the loop, but I want to practice .reduce so I made an array instead */
  const sumVal = sumArray.reduce((total, currentVal)=>total + currentVal, 0);
  return sumVal;
  }
...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(array){
    const displayArr = array.map((element) => {
  ...
  */
  ...
  function USApop(array){
    const population = array.reduce((total, element) => {
      let sum =total + element.population;
      return sum;
    }, 0)
    return population;
  }
...
CuboidMaker.prototype.surfaceArea= function() {
  return (this.length * this.height + this.length * this.width + this.width * this.height)*2
}
